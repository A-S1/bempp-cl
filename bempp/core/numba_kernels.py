        ...
        # where the integration dV becomes (quad_weight * local_test_factor) for a line element.
            ...
            for test_fun_index in range(nshape_test):
                for trial_element_index in range(n_trial_elements):
                    ...
                    for trial_fun_index in range(nshape_trial):
                        for trial_point_index in range(n_quad_points):
                            integrand = ((test_basis_functions[i, test_fun_index, :, test_point_index] @ trial_basis_functions[trial_element_index, trial_fun_index, :, trial_point_index]) -
                                        1/ k2 * (test_basis_divergence[i, test_fun_index, test_point_index] * trial_basis_divergence[trial_element_index, trial_fun_index, trial_point_index])) * kernel_values[trial_element_index * n_quad_points + trial_point_index]
                            local_result[trial_element_index, test_fun_index, trial_fun_index] += integrand * (quad_weights[test_point_index] * local_test_factor * factors[trial_element_index * n_quad_points + trial_point_index])
